...
if results.cterm == 0;
 for i=1:nvar
 if i <= p
        tmp = str2mat(['AR ',num2str(i)]);
        Vname = strvcat(Vname,tmp);
     elseif i <= p+q
        tmp = str2mat(['MA ',num2str(i-p)]);
        Vname = strvcat(Vname,tmp);
     end;
 end;
else
 ...
 if i <= p
        ...
        Vname = strvcat(Vname,tmp);
    elseif i == p+1
        Vname = strvcat(Vname,'Const');
    else   
        tmp = str2mat(['MA ',num2str(i-p-1)]);
        Vname = strvcat(Vname,tmp);
    end;
 end;
end;

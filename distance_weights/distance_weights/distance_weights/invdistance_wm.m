function results = invdistance_wm(lat,long)
% PURPOSE: Creates an inverse distance based spatial weight matrix
%          using latutide and longitude coordinates and 
%          the Great Circle distance formula
% -------------------------------------------------------------------------
% USAGE: result = invdistance_wm(lat,long)
% where: lat  = latitude coordinates (decimal)
%        long = longitude coordinates (decimal)
% -------------------------------------------------------------------------
% RETURNS: a structure
%          results.dista = (n x n) matrix of pairwide distances
%          results.dw    = (n x n) matrix of neighbors (1/distance^2)
% -------------------------------------------------------------------------        
% NOTES: The default measurement for distances is in miles. If you wish to
%        use kilometers, comment out the Earth's radius in miles, line 142
%        and uncomment line 141
%       
...
if nargin ~= 2
error('Wrong # of input arguments to invdistance_wm');
end;
...
temp2 = temp1(2:end,:);
clear temp1
                                     % Grab the second through nth row
...
% e.g. d = 10 would indicate a 10 mile (kilometer) distance cutoff
...
results.dw    = dw;    % Inverse distance weight matrix (1/distance^2)
end
